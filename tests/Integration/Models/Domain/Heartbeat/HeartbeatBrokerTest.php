...
namespace Tests\Integration\Models\Domain\Heartbeat;

use App\Models\Domain\Heartbeat\HeartbeatBroker;
...
final class HeartbeatBrokerTest extends IntegrationTestCase
{
    ...
    public function testUpsertInsertsThenUpdates(): void
    {
        ...
        $this->assertSame('huntress-01', $all[0]->agentId);
        $this->assertSame(12, $all[0]->peerCount);
    }
    ...
    public function testCountOnlineFiltersStale(): void
    {
        ...
        $this->db->query(
            "UPDATE agent_heartbeats SET last_seen = now() - interval '1 hour' WHERE agent_id = ?",
            ['stale-01']
        );
