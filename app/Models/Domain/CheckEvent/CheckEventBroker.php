...
final class CheckEventBroker extends Broker
{
    public function countSince(string $sinceIso): int
    {
        return (int) $this->selectValue(
            "SELECT count(*) FROM check_events WHERE occurred_at >= ?::timestamptz",
            [$sinceIso]
        );
    }
    ...
    public function countByDecisionSince(string $sinceIso): array
    {
        $rows = $this->select(
            "SELECT decision::text AS decision, count(*) AS n
             FROM check_events WHERE occurred_at >= ?::timestamptz
             GROUP BY decision",
            [$sinceIso]
        );
    ...
    public function insert(array $data): int
    {
        ...
        $sql = "INSERT INTO check_events ($colList) VALUES ($placeholders) RETURNING id";
